...
XButton1::
{
	...
	MyGui := Gui()
	MyGui.Add("Edit", "w600 vvCodeV1", Clipboard1)  ; Add a fairly wide edit control at the top of the window.
	MyGui.Add("Button", "default", "Run V2").OnEvent("Click", RunV1)
	MyGui.Add("Edit", "w600 vvCodeV2", ConvertedCode)  ; Add a fairly wide edit control at the top of the window.
	MyGui.Add("Button", "default", "Run V2").OnEvent("Click", RunV2)
	MyGui.Show
	
	return
	RunV1(*){
		TempAhkFile := A_ScriptDir "\testV1.ahk"
	...
	RunV2(*){
		TempAhkFile := A_ScriptDir "\testV2.ahk"
...
WinGetActiveTitle,OutputVar | {1} := WinGetTitle("A")
   ...
   (
	...
	; parse each line of the input script with a array, we do not use length, as we can add or remove lines from the array
...
_Gui(p)
{
	...
	if RegExMatch(GuiLine, "i)^\s*Gui\s*[,\s]\s*.*$"){
		...
		if !InStr(GuiList, "|" GuiNameLine "|"){
			GuiList .= GuiNameLine "|"
			LineResult := GuiNameLine " := Gui()`n" LineResult
		}
		...
		if(Var1="Show"){
			if (RegExCount3){
				LineResult.= GuiNameLine ".Name := " ToStringExpr(Var3) "`n"
				Var3:=""
				RegExCount3:=0
			}
		}
		...
		if(ControlObject!=""){
			LineResult.= "`n" ControlObject ".OnEvent(`"Click`", " ControlLabel ")"
		}
	}
...
_MsgBox(p)
{
	...
	if RegExMatch(p[1], "i)^\dx?\d*\s*"){
		...
		if (text=""){
			ContSection := Convert_GetContSect()
			return format("msgResult := MsgBox, `"`r`n{1}`" ", ContSection )
		}
