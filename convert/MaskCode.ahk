...
; addBlkBraces() - see labelAndFunc.ahk
	...
	if (!TagID && !gaAddedLines.Length) {											; if there are no tags to restore...
		return srcStr																; ... return orig str
	}
	if (TagID) {																	; if a single tag id has been specified by caller...
		addBlkBraces(&srcStr, TagID)												; ... adds braces to (non-brace) if/elseif/else, as needed
		Mask_R(&srcStr, TagID '\w+')												; ... restore all target tags (expand the lines)
		return srcStr																; ... return the expanded code
	}
	addBlkBraces(&srcStr, gaAddedLines)		; array list of all tags				; No tag was specified, add braces for all tags as needed
	for idx, id in gaAddedLines {													; for each tag in list...
		Mask_R(&srcStr, id '\w+')													; ... expand the associated lines
	}
