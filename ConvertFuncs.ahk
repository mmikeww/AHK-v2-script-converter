...
;   global gOScriptStr            := []          ; array of all the lines
   ...
   global gaAddedLines           := []          ; 2025-11-30 AMB, TagIds for condensed lines that are added to script
...
; pre-processing of certain commands via single-iteration of script lines
    ...
    code := Mask_RC(code, 'GOTORET')                                                    ; handle GotoReturn - add braces to IF/ELSEIF/ELSE as needed
...
_ControlGet(p) {
   ...
   out := Mask_TC(out, 'CTRLGET')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
...
_FileCopy(p) {
   ...
   out := RegExReplace(Out, "[\s\,]*\)", ")")
   out := Mask_TC(out, 'FILECOPY')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   Return out
}
;################################################################################
_FileCopyDir(p) {
   ...
   out := RegExReplace(Out, "[\s\,]*\)", ")")
   out := Mask_TC(out, 'FILECOPYDIR')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   Return out
}
;################################################################################
_FileMove(p) {
   ...
   out := RegExReplace(Out, "[\s\,]*\)", ")")
   out := Mask_TC(out, 'FILEMOVE')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   Return out
}
;################################################################################
_FileRead(p) {
...
_FileReadLine(p) {
   ...
   if (gaScriptStrsUsed.ErrorLevel) {
      ...
      Return Mask_TC(cmd, 'FILEREADLN')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   } else {
      out := p[1] " := StrSplit(FileRead(" p[2] "),`"``n`",`"``r`")[" P[3] "]"
      Return Mask_TC(out, 'FILEREADLN')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   }
}
;################################################################################
_FileSelect(p) {
...
;   global gOScriptStr   ; array of all the lines
   ...
   return Mask_TC(Line, 'FILESELECT')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
}
;################################################################################
_FileSetAttrib(p) {
...
_Run(p) {
   ...
   Out := RegExReplace(Out, "[\s\,]*\)$", ")")
   Out := Mask_TC(out, 'RUN')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   Return Out
}
;################################################################################
_SendMessage(p) {
...
_WinGetActiveStats(p) {
   ...
   Out := Mask_TC(Out, 'WINGAS')   ; 2025-11-30 AMB - compress multi-line additions into single-line tag, as needed
   return Out
}
;################################################################################
_WinMove(p) {
