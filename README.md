...
3. Strings can sometimes have words incorrectly replaced, for example `clipboard`, in most cases, will be converted to `A_Clipboard`, regardless of if it's a string or not. This happens with anything present in [convert/5Keywords.ahk](https://github.com/mmikeww/AHK-v2-script-converter/blob/master/convert/5Keywords.ahk), but clipboard is most common.
