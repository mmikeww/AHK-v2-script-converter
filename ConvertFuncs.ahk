...
;  ... and so all code within script has access to these globals prior to _convertLines() call
   ...
   global gmList_HKCmdToFunc     := map()       ; 2025-10-11 - AMB, ADDED - tracks funcs that should be called using 'hotkey' cmd
   global gFuncParams            := ""
...
; Performs tasks that finalize overall conversion
   ...
   addHKCmdCBArgs(&code)                ; 2025-10-11, AMB - Fix #328
...
; 2025-10-11 AMB, UPDATED - to fix issue #328
   LineSuffix := ""
   ...
   if (p[1] = "IfWinActive") {
      ...
      if (SubStr(Trim(P[2]),1,1) = '%') {       ; 2025-10-11 AMB - fix for #328
