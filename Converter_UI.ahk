/*
	...
	2026-03-11 AMB, UPDATED to enable selection of updated/dynamic/auto gui handling
...
class clsUserUI {																					; Gui handling
	...
	_makeGui() {
		...
		this.rdoGuiStd	:= this.oGui.AddRadio('yp checked vStd', 'Updated'						)	; Radio	- Gui Conversion - Updated
		...
		this.rdoGuiStd.OnEvent('Click', this.evGui.bind(this) 									)	; event handler for Radio - Updated (pass entire obj)
	...
	_guiDefaults(mode) {																			; default strings to use for GuiName and CtrlPfx
		switch mode, 0 {
			case 0,1:	return {gName:'myGui',cName:'ogc'}											; for orig or updated	guiMode
	...
	_saveSettings() {																				; saves settings to disk
		...
		if (this.GuiMode <= 1) {																	; if orig or updated mode...
			IniWrite(this.edtGuiName.value,	iniFile, Section, 'GuiStdName'		)					; ... save setting for updated gui name
			IniWrite(this.edtCtrlName.value,iniFile, Section, 'CtrlStdName'		)					; ... save setting for updated ctrl prefix
		}
	...
	_setGuiModeButtons() {																			; update controls related to guiMode
		...
		switch this.GuiMode {
			...
			case 1: this.rdoGuiStd.Value	:= 1													; set updated	guiMode
	...
	_updateGuiModeNames(mode) {																		; gets guiName/ctrlPfx from disk, sets tab controls
		...
		if (mode <= 1) {																			; if orig or updated mode...
